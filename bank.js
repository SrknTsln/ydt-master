...
function renderArsiv() {
    ...
    <button class="bk-nav-cat-btn ${active==='overview'?'bk-cat-open':''}"
            data-action="selectArsivSubList('overview')" style="border-radius:10px;margin-bottom:2px;">
    ...
    cats.forEach(cat => {
        ...
        <div class="bk-nav-cat ${cat.theme}">
            <button class="bk-nav-cat-btn ${isOpen?'bk-cat-open':''}"
                    data-action="bkToggleCat('${cat.key}')">
            ...
            <div class="bk-nav-sub ${isOpen?'bk-sub-open':''}">
                ${cat.subs.map(sub => sub.isImport
                    ? `<button class="bk-nav-add-btn" data-action="showImportParagrafModal()">＋ Paket Yükle</button>`
                    : `<button class="bk-nav-sub-btn ${active===sub.key?'bk-sub-active':''}"
                                data-action="selectArsivSubList('${sub.key}')">
    ...
    ydt_cats.forEach(cat => {
        ...
        <div class="bk-nav-cat ${cat.theme}">
            <button class="bk-nav-cat-btn ${isActive?'bk-cat-open':''}"
                    data-action="selectArsivSubList('${cat.key}')">
                <div class="bk-nav-cat-icon">${cat.icon}</div>
                <div class="bk-nav-cat-info">
                    <span class="bk-nav-cat-name">${cat.label}</span>
    ...
    if (mobBar) {
        ...
        mobBar.innerHTML = allNavCats.map(c =>
            `<button class="bk-mob-sub-pill ${active===c.key||active.startsWith(c.key)?'bk-mob-active':''} ${c.theme||''}"
                     data-action="selectArsivSubList('${c.key}')">
    ...
    if (active === 'overview') {
        ...
        ovItems.forEach(item => {
            ovHtml += `
            <div class="bk-ov-card ${item.theme}" data-action="selectArsivSubList('${item.key}')">
    ...
    if (gramerMatch) {
        ...
        gActive.forEach((item, idx) => {
            ...
            <div class="bk-q-card ${revealed ? (userAns===item.correct?'bk-correct':'bk-wrong') : ''}">
                <div class="bk-q-meta-row">
                    ...
                    ${revealed ? `<button class="bk-q-reset" data-action="resetBankQuestion('${qId}')" style="display:inline-flex;">↺ Tekrar</button>` : ''}
                </div>
                ...
                <div class="bk-opts">
                    ${Object.entries(item.options||{}).map(([k,v]) => {
                        ...
                        return `<button class="bk-opt ${cls}" data-action="bankSelectGramer('${qId}','${k}')" ${revealed?'disabled':''}>
                ...
                ${!revealed ? `<div style="padding:0 16px 14px;"><button data-action="bankRevealGramer('${qId}')"
        ...
        if (gGroups > 1) {
            gHtml += `<div class="bk-pager">
                <button class="bk-pg-btn" data-action="setArsivGroupPage(${gGroupPg-1})" ${gGroupPg<=1?'disabled':''}>‹</button>`;
            for (let p=1; p<=gGroups; p++) {
                gHtml += `<button class="bk-pg-btn ${p===gGroupPg?'bk-pg-act':''}" data-action="setArsivGroupPage(${p})">${(p-1)*BANK_GROUP_SIZE+1}–${Math.min(p*BANK_GROUP_SIZE,gSlice.length)}</button>`;
            }
            gHtml += `<button class="bk-pg-btn" data-action="setArsivGroupPage(${gGroupPg+1})" ${gGroupPg>=gGroups?'disabled':''}>›</button></div>`;
        }
    ...
    if (paraGMatch) {
        ...
        if (pData?.questions?.length) {
            ...
            <div class="bk-pasaj-wrap">
                <div class="bk-pasaj-hdr">
                    ...
                    <button class="bk-pasaj-goto" data-action="goToPasajOku()">📖 Okuma Modu →</button>
                </div>
                    ...
                    <div class="bk-opts" id="opts_${qId}">
                        ${(q.options||[]).map(opt=>{
                            ...
                            return `<button class="bk-opt sb-opt-btn" id="opt_${qId}_${letter}"
                                data-action="solveBankQuestion('${qId}','${letter}')">
        ...
        if (groupKeys.length > 1) {
            pHtml += `<div class="bk-pager">
                <button class="bk-pg-btn" data-action="setParagrafGroupPage('${pgKey}',${curPage-1})" ${curPage<=1?'disabled':''}>‹</button>`;
            groupKeys.forEach((_, gi) => {
                const pg = gi+1;
                pHtml += `<button class="bk-pg-btn ${pg===curPage?'bk-pg-act':''}" data-action="setParagrafGroupPage('${pgKey}',${pg})">${startIdx+pg}</button>`;
            });
            pHtml += `<button class="bk-pg-btn" data-action="setParagrafGroupPage('${pgKey}',${curPage+1})" ${curPage>=groupKeys.length?'disabled':''}>›</button></div>`;
        }
    ...
    if (kelMatch) {
        ...
        activeItems.forEach((q, idx) => {
            ...
            const optsHtml = Object.entries(q.options||{}).map(([k,v]) => {
                ...
                return `<button class="bk-opt sb-opt-btn ${cls}" id="opt_${qId}_${k}"
                    data-action="solveBankQuestion('${qId}','${k}')" ${revealed?'disabled':''}>
                    <span class="bk-opt-key">${k}</span>
                    <span class="bk-opt-text">${v}</span>
                    ${revealed&&isC?'<span style="margin-left:auto">✅</span>':''}
                    ${revealed&&isCh&&!isC?'<span style="margin-left:auto">❌</span>':''}
            ...
            <div class="bk-q-card ${revealed?(userAns===q.correct?'bk-correct':'bk-wrong'):''}">
                <div class="bk-q-meta-row">
                    ...
                    ${revealed ? `<button class="bk-q-reset" id="reset_${qId}" data-action="resetBankQuestion('${qId}')" style="display:inline-flex;">↺ Tekrar</button>` : ''}
                </div>
                <div class="bk-q-text">${q.question}</div>
        ...
        if (totalGroups > 1) {
            html += `<div class="bk-pager">
                <button class="bk-pg-btn" data-action="setArsivGroupPage(${groupPage-1})" ${groupPage<=1?'disabled':''}>‹</button>`;
            for (let p=1; p<=totalGroups; p++) {
                const gs = (p-1)*BANK_GROUP_SIZE+1, ge = Math.min(p*BANK_GROUP_SIZE, subItems.length);
                html += `<button class="bk-pg-btn ${p===groupPage?'bk-pg-act':''}" data-action="setArsivGroupPage(${p})">${gs}–${ge}</button>`;
            }
            html += `<button class="bk-pg-btn" data-action="setArsivGroupPage(${groupPage+1})" ${groupPage>=totalGroups?'disabled':''}>›</button></div>`;
        }
