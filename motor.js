...
function showPage(id) {
    ...
    // container olmayan tam sayfa elemanları da gizle
...
function showStatsPage() {
    let total = 0, learned = 0, allWords = [];
    ...
    let topErrors = allWords.sort((a, b) => (b.errorCount || 0) - (a.errorCount || 0))
        .slice(0, 5).filter(w => (w.errorCount || 0) > 0);
    document.getElementById('error-list').innerHTML = topErrors.length
        ? topErrors.map(w => `<li><strong>${w.eng}</strong> (${w.errorCount} Hata)</li>`).join('')
        : "<li>Kayıt yok.</li>";
    ...
    hmSel.innerHTML = '';
    Object.keys(allData).forEach(n => hmSel.add(new Option(n, n)));
    renderHeatmap();
...
function startTypingQuiz() {
    const listName = document.getElementById('exercise-list-selector').value;
    if (!allData[listName] || !allData[listName].length) return alert('Liste boş!');
...
function renderArsiv() {
    ...
    document.getElementById('arsiv-summary').innerHTML = `
        <div style="margin-bottom:18px;">
            <div style="font-size:.6rem;font-weight:800;text-transform:uppercase;letter-spacing:1.2px;color:var(--ink3);margin-bottom:10px;">📊 Soru Bankası İstatistikleri</div>

            <!-- Üst sıra: 3 kart yan yana (sadece başlık+sayı, overflow yok) -->
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:0;">

                <!-- KELİME KARTI BAŞLIĞI -->
                <button onclick="toggleKelimeCard()"
                    style="flex:1;min-width:180px;max-width:260px;background:var(--white);border:1.5px solid ${kelimeCardOpen ? '#6366f1' : '#e0e7ff'};border-radius:${kelimeCardOpen ? '14px 14px 0 0' : '14px'};padding:14px 16px;cursor:pointer;font-family:inherit;text-align:left;position:relative;overflow:hidden;">
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                        <div style="font-size:.6rem;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:#6366f1;">📝 Kelime Soruları</div>
                        <span style="font-size:.8rem;color:#6366f1;display:inline-block;transform:${kelimeCardOpen ? 'rotate(180deg)' : ''};">▼</span>
                    </div>
                    <div style="font-size:2rem;font-weight:900;color:var(--ink);line-height:1;">${totalKelime}</div>
                    <div style="font-size:.68rem;color:var(--ink3);margin-top:4px;">${kelimePages} liste · ${BANK_PAGE_SIZE} soru/liste</div>
                </button>

                <!-- PARAGRAF KARTI BAŞLIĞI -->
                <button onclick="toggleParagrafBankCard()"
                    style="flex:1;min-width:180px;max-width:260px;background:var(--white);border:1.5px solid ${paragrafCardOpen ? '#10b981' : '#d1fae5'};border-radius:${paragrafCardOpen ? '14px 14px 0 0' : '14px'};padding:14px 16px;cursor:pointer;font-family:inherit;text-align:left;position:relative;overflow:hidden;">
                    <div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#10b981,#059669);"></div>
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                        <div style="font-size:.6rem;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:#10b981;">📖 Paragraf Soruları</div>
                        <span style="font-size:.8rem;color:#10b981;display:inline-block;transform:${paragrafCardOpen ? 'rotate(180deg)' : ''};">▼</span>
                    </div>
                    <div style="font-size:2rem;font-weight:900;color:var(--ink);line-height:1;">${totalParagraf}</div>
                    <div style="font-size:.68rem;color:var(--ink3);margin-top:4px;">${paragrafPasajSayisi} pasaj · ${paraGroupCount} grup</div>
                </button>

                <!-- GRAMER KARTI BAŞLIĞI -->
                <button onclick="toggleGramerCard()"
                    style="flex:1;min-width:180px;max-width:260px;background:var(--white);border:1.5px solid ${gramerCardOpen ? '#f59e0b' : '#fef3c7'};border-radius:${gramerCardOpen ? '14px 14px 0 0' : '14px'};padding:14px 16px;cursor:pointer;font-family:inherit;text-align:left;position:relative;overflow:hidden;">
                    <div style="position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,#f59e0b,#d97706);"></div>
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                        <div style="font-size:.6rem;font-weight:800;text-transform:uppercase;letter-spacing:1px;color:#f59e0b;">⚙️ Gramer Soruları</div>
                        <span style="font-size:.8rem;color:#f59e0b;display:inline-block;transform:${gramerCardOpen ? 'rotate(180deg)' : ''};">▼</span>
                    </div>
                    <div style="font-size:2rem;font-weight:900;color:var(--ink);line-height:1;">${totalGramer}</div>
                    <div style="font-size:.68rem;color:var(--ink3);margin-top:4px;">${gramerPages} liste · ${BANK_PAGE_SIZE} soru/liste</div>
                </button>

            </div>

            <!-- Alt alan: Açık panelin içeriği tam genişlikte, kartların ALTINDA -->
            ${kelimeCardOpen ? `
            <div style="background:var(--bg);border:1.5px solid #6366f1;border-top:none;border-radius:0 0 12px 12px;padding:8px 8px 4px;margin-bottom:4px;">
                ${subBtnsHtml}
            </div>` : ''}
            ${paragrafCardOpen ? `
            <div style="background:var(--bg);border:1.5px solid #10b981;border-top:none;border-radius:0 0 12px 12px;padding:8px 8px 4px;margin-bottom:4px;">
                ${paragrafSubBtnsHtml}
            </div>` : ''}
            ${gramerCardOpen ? `
            <div style="background:var(--bg);border:1.5px solid #f59e0b;border-top:none;border-radius:0 0 12px 12px;padding:8px 8px 4px;margin-bottom:4px;">
                ${gramerSubBtnsHtml}
            </div>` : ''}
