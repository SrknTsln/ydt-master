...
function renderWeakWords() {
    ...
    el.innerHTML = top5.map((w, i) => {
        ...
        return `
        <div class="idx-weak-item ${danger}" data-speak="${w.eng.replace(/"/g, '&quot;')}">
...
async function renderBankStats() {
    ...
    el.innerHTML = rows.map(r => {
        ...
        return `
        <div class="idx-bank-row" data-action="${r.onclick}">
...
function renderGrammarProgress() {
    ...
    el.innerHTML = GRAMMAR_MODULES.map(m => {
        ...
        return `
        <div class="idx-gr-row" data-action="${m.fnName||m.fn}" title="${m.label} — ${score}/${m.total}">
...
function renderSM2Plan() {
    ...
    <div class="idx-sm2-summary">
        <div class="idx-sm2-urgent" style="border-color:${urgentColor};background:${urgentColor}12;" data-action="startSM2Review()">
...
// ── Window exports — motor.js'in defer sonrası çağırabilmesi için ──
window.renderWeakWords      = renderWeakWords;
window.renderPerfChart      = renderPerfChart;
window.renderPOSChart       = renderPOSChart;
window.renderSoruStats      = renderSoruStats;
window.renderBankStats      = renderBankStats;
window.renderGrammarProgress = renderGrammarProgress;
window.renderSM2Plan        = renderSM2Plan;
